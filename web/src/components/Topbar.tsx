import { Bell, ChevronDown, ExternalLink, KeyRound, Menu, Search } from "lucide-react";
...
import { Flag, StatusCode } from "./ui";
...
export function Topbar() {
  const { edge, identity, events } = useStore();
  const alerts = events.filter((e) => e.verdict === "blocked_waf" || e.verdict === "rate_limited");
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[60px] items-center gap-3 border-b border-line bg-ink-950/80 px-4 backdrop-blur-md lg:px-5">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <div className="relative">
          <button className="btn-ghost relative h-9 w-9 px-0" aria-label={`Alerts: ${alerts.length}`} onClick={() => setMenu(menu === "alerts" ? null : "alerts")} aria-expanded={menu === "alerts"}>
            <Bell className="size-4" />
            {alerts.length > 0 && <span className="absolute -top-1 -right-1 grid min-w-4 place-items-center rounded-full bg-brand px-1 text-[9.5px] font-bold text-white">{alerts.length}</span>}
          </button>
          <Popover open={menu === "alerts"} onClose={() => setMenu(null)} className="w-80">
            <div className="label px-2 pt-1 pb-1.5">Edge blocks · this session</div>
            {alerts.length === 0 && <p className="px-2 py-3 text-[12px] text-dim">No blocked requests yet. Try “Blocked Request” or “Rate Limited”.</p>}
            {alerts.slice(0, 6).map((a) => (
              <button key={a.id} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left hover:bg-white/5" onClick={() => { setMenu(null); window.history.pushState(null, "", `/logs?q=${encodeURIComponent(a.ray ?? "")}`); navigate("/logs"); }}>
                <StatusCode code={a.status} />
                <span className="min-w-0 flex-1"><span className="block truncate font-mono text-[11.5px]">{a.path}</span><span className="block text-[11px] text-dim">{a.verdict === "blocked_waf" ? "Blocked by WAF" : "Rate limited"} · {ago(a.t)}</span></span>
              </button>
            ))}
          </Popover>
        </div>
        ...
        <div className="relative">
          <button className="flex h-10 items-center gap-2.5 rounded-lg px-2 hover:bg-white/[0.03]" onClick={() => setMenu(menu === "user" ? null : "user")} aria-expanded={menu === "user"}>
            <span className="grid size-8 place-items-center rounded-full bg-gradient-to-br from-[#2f80ed] to-[#8b5cf6] text-[12px] font-bold text-white ring-2 ring-line-strong">DH</span>
